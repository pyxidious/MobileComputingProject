...
namespace PoliceChase
{
	...
	public class GameController : MonoBehaviour
	{
        ...
        public GameObject Player_Spawnpoint;
        void Awake()
		{
            Time.timeScale = 1;
        ...
        public void StartGame()
        {
            ...
            if (playerObject)
            {
                ...
                Vector3 spawnPosition = Player_Spawnpoint.transform.position;
                playerObject = Instantiate(playerObject, spawnPosition, Quaternion.identity);
            ...
            if (GetComponent<SpawnAroundObjects>()) GetComponent<SpawnAroundObjects>().isSpawning = true;
            if (GetComponent<SpawnOnPoints>()) GetComponent<SpawnOnPoints>().isSpawning = true;
				...
				else
				{
                    ...
                    if (playerObject)
                    {
                        ...
                        else // Otherwise, use gamepad/keyboard controls
                        {
                            ...
                            if ( directionalControls)    playerObject.targetPosition = playerObject.transform.position + new Vector3(Input.GetAxis("Horizontal") * 5, playerObject.transform.position.y, Input.GetAxis("Vertical") * 5);
                            else    playerDirection = Input.GetAxis("Horizontal");
                        }
        ...
        public void dieEvents()
        {
            // Invoca il metodo per caricare la scena del Game Over dopo 5 secondi
            Invoke("LoadGameOverScene", 2f);
        }

        // Metodo che carica la scena del Game Over
